...
class NarrativeGenerator:
    ...
    async def narrate(self, tick_result: TickResult, world: WorldState) -> str:
        ...
        if self.llm:
            try:
                narrative = await self._llm_narrate(context, tick_result, world)
                return narrative
            except Exception as e:
                logger.warning("LLM narration failed: %s, using fallback", e)

        return self._fallback_narrate(context, tick_result, world)
    ...
    async def _llm_narrate(
        self, context: dict[str, Any], result: TickResult, world: WorldState | None = None
    ) -> str:
    ...
    def _fallback_narrate(
        ...
        if action:
            ...
            if action_type == "move" and target:
                target_loc_name = target
                if world:
                    target_loc = world.locations.get(target)
                    if target_loc:
                        target_loc_name = target_loc.name
                parts.append(f"你动身前往{target_loc_name}。")
